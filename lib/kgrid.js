/* THE KINETIC GRID — portable.
   ...
   7. TWO TOKENS, FROM THE HOST. --kgrid-ink is the colour as a bare r,g,b
      triplet; --kgrid-rest scales the resting alphas so the same grid reads
      the same weight on a light ground as on an ink one. Both are read off
      the host element, so folds on opposite grounds can each set their own.
   ...
   ---------------------------------------------------------------------------- */
(function kgrid(host) {
  /* Named, and it calls itself once per host. querySelector was singular, which
     was fine while the ink CTA was the only fold that wanted a grid — the moment
     a second one does, the first match wins and the other silently loses the
     device it already had. A named function expression takes the whole body as
     it stands and mounts it per host with no re-indentation and no shared
     state: each grid gets its own canvas, its own ripples, its own loop. */
  if (host === undefined) {
    var all = document.querySelectorAll('[data-kgrid]');
    for (var h = 0; h < all.length; h++) kgrid(all[h]);
    return;
  }
  ...
  var DOT = 28;             /* pitch of that texture                           */
  var EASE = 0.08;          /* pointer follow, per frame                       */
  var WAVE = 55;            /* width of a ripple's displacement band           */
  var R_BASE = 1.8, R_ACTIVE = 3.2;

  /* FROM THE HOST, NOT THE ROOT. It was written as a root lookup and described
     in kgrid.css as the way "a host on a light ground can flip it" — which it
     was not: read off documentElement it is one value for the whole page, so
     two folds on opposite grounds cannot both be served. Read off the host it
     is an ordinary inherited custom property and each fold sets its own.

     --kgrid-rest scales the RESTING alphas only, and it exists because the same
     alpha is not the same grid on the two grounds. Composited and measured:
     white at .13 over ink is 1.47:1 against its ground, ink at .13 over chalk is
     1.30:1 — the light grid comes out visibly fainter at identical numbers. The
     alphas that restore parity are .184, .292, .066 and .408, which is 1.42x
     across the board, so one multiplier covers it rather than four values.

     The HOT ends are deliberately not scaled. Both saturate — .9 white on ink
     is 14.54:1, .9 ink on chalk is 11.87:1 — so there is nothing to correct,
     and a host that wants a quieter highlight should say so in its own value
     rather than have this multiplier decide it. */
  var cs = getComputedStyle(host);
  var ink  = (cs.getPropertyValue('--kgrid-ink') || '255,255,255').trim();
  var rest = parseFloat(cs.getPropertyValue('--kgrid-rest')) || 1;

  var A_LINE = 0.13 * rest, A_LINE_HOT = 0.9;
  var A_NODE = 0.20 * rest, A_NODE_HOT = 1.0;
  var A_DOT  = 0.05 * rest;     /* the still texture behind the grid */
  var A_RING = 0.28 * rest;     /* a ripple's own outline           */
  ...
  function draw(now) {
    ...
    ctx.fillStyle = 'rgba(' + ink + ',' + A_DOT.toFixed(3) + ')';
    ...
    for (i = 0; i < ripples.length; i++) {
      ...
      ctx.strokeStyle = 'rgba(' + ink + ',' + (ripples[i].opacity * A_RING).toFixed(3) + ')';
      ctx.lineWidth = 1.5;
      ctx.stroke();
    }
  }
